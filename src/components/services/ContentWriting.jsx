                ...
                <div className="container mx-auto max-w-[1400px] relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div>
                        ...
                        <div className="flex flex-wrap gap-4">
                            <a href={calendlyLink} target="_blank" rel="noopener noreferrer">
                                <PrimaryButton className="!px-8 !py-4 text-lg">Order Content</PrimaryButton>
                            </a>
                            <Link to="/portfolio">
                                <button className="px-8 py-4 rounded-xl border border-white/20 text-white font-medium hover:bg-white hover:text-black transition-all">View Samples</button>
                            </Link>
                        </div>
                    </div>
                    <div className="relative group">
                        ...
                        <div className="relative rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl bg-[#0f172a]">
                            <img src={d('heroImage', contentHero)} alt="Hero" className="w-full h-auto object-cover transform group-hover:scale-105 transition-transform duration-1000" />
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="py-16 bg-[#0f172a] border-y border-white/5">
                <div className="container mx-auto px-6 lg:px-16 max-w-[1400px]">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                        <div className="p-6">
                            <h3 className="text-4xl font-bold text-white mb-2">{d('feature1Title', 'Authority')}</h3>
                            <p className="text-gray-400">{d('feature1Desc', 'Establish trust with expert-level, well-researched articles.')}</p>
                        </div>
                        <div className="p-6 border-y md:border-y-0 md:border-x border-white/10">
                            <h3 className="text-4xl font-bold text-white mb-2">{d('feature2Title', 'Visibility')}</h3>
                            <p className="text-gray-400">{d('feature2Desc', 'Rank for high-intent keywords that bring ready-to-buy traffic.')}</p>
                        </div>
                        <div className="p-6">
                            <h3 className="text-4xl font-bold text-white mb-2">{d('feature3Title', 'Loyalty')}</h3>
                            <p className="text-gray-400">{d('feature3Desc', 'Turn casual readers into brand advocates with a unique voice.')}</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="py-32 px-6 lg:px-16">
                <div className="container mx-auto max-w-[1400px]">
                    <div className="text-center mb-20">
                        <h2 className="text-4xl lg:text-5xl font-bold text-white mb-6">{d('offeringsTitle', 'Content That Covers Every Stage')}</h2>
                                ...
                                <div className="mt-10 pt-8 border-t border-white/10">
                                    <a href={calendlyLink} target="_blank" rel="noopener noreferrer">
                                        <PrimaryButton className="w-full justify-center !py-4 text-lg">
                                            {d('checklistCta', 'Start Your Content Strategy')}
                                        </PrimaryButton>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                {(pageData?.reasons || defaults?.reasons || []).map((reason, i) => (
                                    <div key={i} className="flex items-start gap-4 p-4 rounded-xl bg-white/5 border border-white/10">
                                        <div className="text-red-400 font-bold text-xl">0{i+1}</div>
                                        <div>
                    ...
                    <div className="space-y-4">
                        {(pageData?.faqs || defaults?.faqs || []).map((faq, idx) => (
                            <details key={idx} className="group bg-white/5 rounded-2xl border border-white/10 overflow-hidden">
                                ...
                                <div className="px-6 pb-6 text-gray-400 leading-relaxed whitespace-pre-wrap">
                                    {faq.answer}
                                </div>
                            </details>
                        ))}
