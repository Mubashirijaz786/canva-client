...
const CustomSoftware = () => {
    ...
    const defaults = PAGE_DEFAULTS['custom-software'];

    useEffect(() => {
        const fetchPageContent = async () => {
            try {
                const res = await axiosPublic.get('/service-pages/custom-software');
                setPageData(res.data);
            } catch (err) {
                console.log("Using default Software content.");
            } finally {
                setLoading(false);
            }
        };
                ...
                <div className="container mx-auto max-w-[1400px] relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div>
                        ...
                        <h1 className="text-5xl lg:text-7xl font-bold font-['Manrope'] mb-6 leading-[1.1] text-white">
                            {d('heroTitle')} <br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-violet-500">{d('heroSubtitle')}</span>
                        </h1>
                        <p className="text-gray-400 text-xl leading-relaxed mb-10 max-w-xl">{d('heroDescription')}</p>
                        <div className="flex flex-wrap gap-4">
                            <a href={calendlyLink} target="_blank" rel="noopener noreferrer"><PrimaryButton className="!px-8 !py-4 text-lg">Consult An Architect</PrimaryButton></a>
                            <Link to="/portfolio"><button className="px-8 py-4 rounded-xl border border-white/20 text-white font-medium hover:bg-white hover:text-black transition-all">View Case Studies</button></Link>
                        </div>
                        <div className="mt-12 flex gap-8 border-t border-white/10 pt-8">
                            <div><h4 className="text-3xl font-bold text-white">100%</h4><p className="text-gray-500 text-sm">IP Ownership</p></div>
                            <div><h4 className="text-3xl font-bold text-white">24/7</h4><p className="text-gray-500 text-sm">System Monitoring</p></div>
                        </div>
                    </div>
                    <div className="relative group">
                        ...
                        <div className="relative rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl bg-[#0f172a]">
                            <img src={d('heroImage', softwareHero)} alt="Hero" className="w-full h-auto object-cover transform group-hover:scale-105 transition-transform duration-1000" />
                            <div className="absolute bottom-8 right-8 bg-black/80 backdrop-blur-md p-4 rounded-2xl border border-white/10 flex items-center gap-4 shadow-xl">
                                <div className="w-12 h-12 rounded-full bg-indigo-500 flex items-center justify-center text-black"><Cpu size={24} fill="black" /></div>
                                <div>
                ...
                <div className="container mx-auto px-6 lg:px-16 max-w-[1400px]">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
                        {techStack.map((tech, idx) => (
                            <div key={idx} className="flex flex-col items-center justify-center p-6 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all cursor-default group">
                            ...
                            <div className="relative p-8 rounded-3xl bg-[#020617] border border-indigo-500/30 shadow-2xl">
                                ...
                                <div className="mt-10 pt-8 border-t border-white/10">
                                    <a href={calendlyLink} target="_blank" rel="noopener noreferrer">
                                        <PrimaryButton className="w-full justify-center !py-4 text-lg">Estimate My Project</PrimaryButton>
                                    </a>
                                </div>
                            </div>
                        </div>
