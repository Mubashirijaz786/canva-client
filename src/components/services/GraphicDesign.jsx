...
const GraphicDesign = () => {
    ...
    const defaults = PAGE_DEFAULTS['graphic-design'];

    useEffect(() => {
        const fetchPageContent = async () => {
            try {
                const res = await axiosPublic.get('/service-pages/graphic-design');
                ...
                <div className="container mx-auto max-w-[1400px] relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div>
                        ...
                        <h1 className="text-5xl lg:text-7xl font-bold font-['Manrope'] mb-6 leading-[1.1] text-white">
                            {d('heroTitle')} <br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500">{d('heroSubtitle')}</span>
                        </h1>
                    ...
                    <div className="relative group">
                        ...
                        <div className="relative rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl bg-[#0f172a]">
                            <img src={d('heroImage', graphicHero)} alt="Hero" className="w-full h-auto object-cover transform group-hover:scale-105 transition-transform duration-1000" />
                            <div className="absolute bottom-8 right-8 bg-black/80 backdrop-blur-md p-4 rounded-2xl border border-white/10 flex items-center gap-4 shadow-xl">
                                <div className="w-12 h-12 rounded-full bg-orange-500 flex items-center justify-center text-black"><Palette size={24} fill="black" /></div>
                                <div>
                                ...
                                {(pageData?.reasons || defaults?.reasons || []).map((item, i) => (
                                    <div key={i} className="flex gap-6">
                                        <div className="flex flex-col items-center">
                                            <div className="w-10 h-10 rounded-full bg-orange-500 text-black font-bold flex items-center justify-center shrink-0">{i + 1}</div>
