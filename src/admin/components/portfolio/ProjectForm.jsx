            ...
            <div className="relative w-full max-w-2xl bg-[#0f172a] border border-white/10 rounded-[3rem] p-8 md:p-10 shadow-2xl animate-in zoom-in-95 overflow-y-auto max-h-[90vh]">
                ...
                <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }} className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <select
                            className="bg-[#0f172a] border border-white/10 p-4 rounded-2xl text-white outline-none focus:border-blue-500 cursor-pointer font-bold"
                    ...
                    <textarea placeholder="Description..." required className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-white h-24 outline-none focus:border-blue-500 resize-none" value={formData.desc} onChange={(e) => setFormData({ ...formData, desc: e.target.value })} />
                    ...
                    <div className="flex items-center gap-6">
                        <label className="flex-grow bg-white/5 border border-dashed border-white/20 p-8 rounded-2xl cursor-pointer hover:bg-white/10 transition-all text-center group">
                            ...
                            <span className="text-sm text-gray-400 font-bold uppercase tracking-widest">Select Project Image</span>
                            <input type="file" className="hidden" accept="image/*" onChange={handleFileChange} />
                        </label>
                        
                        {imagePreview && (
                            <div className="relative shrink-0">
                                <img src={imagePreview} className="w-24 h-24 object-cover rounded-2xl border border-white/10 shadow-xl" alt="Preview" />
                    ...
                    <div className="space-y-2">
                        <div className="flex gap-2">
                            <input 
                                ...
                                className="flex-grow bg-white/5 border border-white/10 p-3 rounded-xl text-white outline-none focus:border-blue-500" 
                                value={currentTag} 
                                onChange={(e) => setCurrentTag(e.target.value)} 
                                onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addTag(); } }}
                            />
                            <button type="button" onClick={addTag} className="bg-blue-600 px-4 rounded-xl text-white"><Plus /></button>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            {formData.tags.map((t, i) => (
                                <span key={i} className="bg-blue-500/10 text-blue-400 text-[10px] font-black px-3 py-1 rounded-lg border border-blue-500/20 flex items-center gap-1 uppercase tracking-tighter">
                                    {t} <X size={10} className="cursor-pointer hover:text-red-500" onClick={() => setFormData({ ...formData, tags: formData.tags.filter((tag) => tag !== t) })} />
                                </span>
                            ))}
                        </div>
                    </div>

                    <button disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 py-5 rounded-2xl font-black text-white shadow-xl transition-all active:scale-95 disabled:opacity-50">
