...
import { Save, Loader2, Phone, Mail, Share2, Plus, Trash2, Calendar, MessageCircle } from 'lucide-react';
...
const ManageSettings = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        console.error("Failed to load ecosystem settings.");
      }
    };
    fetchSettings();
  }, []);
  ...
  return (
    <div className="max-w-5xl animate-in fade-in duration-500 pb-20 px-4 font-['Manrope']">
      <h1 className="text-3xl font-black italic text-white mb-8 uppercase tracking-tighter text-blue-500">Ecosystem Settings</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="bg-gradient-to-br from-green-500/10 to-blue-500/10 border border-white/10 p-8 rounded-[2.5rem] space-y-6 shadow-2xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
            ...
            <div className="md:col-span-2 space-y-2">
              <label className="text-[10px] text-gray-500 font-black uppercase flex items-center gap-2 ml-1">
                <MessageCircle size={12} className="text-green-400" /> WhatsApp Default Message
              </label>
              <input
                type="text"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="p-8 bg-white/[0.03] border border-white/10 rounded-[2.5rem] space-y-6 shadow-xl">
            ...
            <div className="space-y-3">
              <label className="text-[10px] text-gray-500 font-black uppercase ml-1">Phone Numbers</label>
              ...
              <button type="button" onClick={() => addField('phones')} className="text-blue-500 text-[10px] font-bold flex items-center gap-1 hover:underline uppercase tracking-wider"><Plus size={12} /> Add Phone</button>
            </div>

            <div className="space-y-3">
              <label className="text-[10px] text-gray-500 font-black uppercase ml-1">Emails</label>
              ...
              <button type="button" onClick={() => addField('emails')} className="text-blue-500 text-[10px] font-bold flex items-center gap-1 hover:underline uppercase tracking-wider"><Plus size={12} /> Add Email</button>
            </div>

            <div className="space-y-3">
              <label className="text-[10px] text-gray-500 font-black uppercase ml-1">Locations</label>
              ...
              <button type="button" onClick={() => addField('addresses')} className="text-blue-500 text-[10px] font-bold flex items-center gap-1 hover:underline uppercase tracking-wider"><Plus size={12} /> Add Location</button>
            </div>
          </div>

          <div className="p-8 bg-white/[0.03] border border-white/10 rounded-[2.5rem] space-y-4 shadow-xl">
            ...
            <div className="space-y-4 mt-4">
              {['facebook', 'instagram', 'linkedin', 'upwork', 'copyright'].map((field) => (
                <div key={field} className="space-y-1">
                  <label className="text-[10px] text-gray-500 font-black uppercase ml-1 tracking-widest">{field}</label>
                  <input
                    type="text"
        ...
        <button disabled={loading} className="bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 text-white px-12 py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-xs flex items-center gap-3 shadow-xl active:scale-95 transition-all shadow-blue-600/20">
